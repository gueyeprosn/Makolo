...
export const demoProfiles: Profile[] = [
  ...
  },
  {
    ...
    verification_note: null,
    verification_requested_at: iso(30),
    verified_at: iso(28),
    created_at: iso(150),
    updated_at: iso(3),
  },
  {
    ...
    verification_note: null,
    verification_requested_at: iso(2),
    verified_at: null,
    created_at: iso(140),
    updated_at: iso(8),
  },
  ...
  },
  {
    ...
    verification_note: "Pièce d'identité illisible — merci de renvoyer une photo nette du recto et du verso.",
    verification_requested_at: iso(10),
    verified_at: null,
    created_at: iso(100),
    updated_at: iso(9),
  },
...
export const demoListings: Listing[] = seeds.map((seed) => {
  ...
  return {
    ...
    // Seules les annonces publiées ou refusées sont passées par une vraie
    // décision de modération dans ce jeu de démonstration.
    moderated_at: seed.status === 'published' || seed.status === 'rejected' ? iso(Math.max(0, seed.days - 1)) : null,
